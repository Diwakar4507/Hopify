...
class PatientDashboard(BaseWindow):
    ...
    def __init__(self):
        ...
        self.patient = {
            ...
            "gender":  "Male",
            "blood":   "B+",
            "phone":   "+977-9842066145",
            "email":   "diwakarsharma0111@gmail.com",
    ...
    def _build_sidebar(self, parent):
        ...
        tk.Label(logo_frame, text="🏥", bg=CLR_SIDEBAR,
                 font=("Segoe UI Emoji", 22)).pack(side=tk.LEFT)
        tk.Label(logo_frame, text=" HOPIFY", bg=CLR_SIDEBAR,
                 fg=CLR_WHITE, font=FONT_LOGO).pack(side=tk.LEFT)

        tk.Label(sb, text="Patient Portal",
                 bg=CLR_SIDEBAR, fg="#7A94BE",
                 font=FONT_SMALL).pack(anchor=tk.W, padx=22, pady=(0, 14))
        ...
        tk.Label(user_frame, text="👤", bg=CLR_SIDEBAR_LT,
        ...
        display_name = name_parts[0] + " " + name_parts[-1] if len(name_parts) > 1 else name_parts[0]
        
        tk.Label(info, text=display_name,
                 bg=CLR_SIDEBAR_LT, fg=CLR_WHITE,
                 font=("Segoe UI", 10, "bold")).pack(anchor=tk.W)
        tk.Label(info, text=self.patient["id"],
                 bg=CLR_SIDEBAR_LT, fg="#7A94BE",
    ...
    def _nav_item(self, parent, icon, label):
        ...
        icon_lbl = tk.Label(frame, text=icon, bg=CLR_SIDEBAR,
                            fg=CLR_WHITE, font=("Segoe UI Emoji", 13))
        icon_lbl.pack(side=tk.LEFT, padx=(0, 10))

        text_lbl = tk.Label(frame, text=label, bg=CLR_SIDEBAR,
                            fg="#A8C0E0" if not is_logout else "#EF8080",
                            font=FONT_NAV, anchor=tk.W)
    ...
    def set_active(self, lbl):
        """Clean class-level navigation router handling styling and pages."""
        ...
        for btn_lbl, (btn_f, btn_t, btn_i, btn_a) in self._nav_buttons.items():
            ...
            btn_t.config(bg=CLR_SIDEBAR,
                         fg="#A8C0E0" if btn_lbl != "Logout" else "#EF8080",
                         font=FONT_NAV)
    ...
    def _activate_nav(self, label):
        """Pass-through legacy reference map redirecting to the true handler."""
    ...
    def _build_topbar(self, parent):
        ...
        self.topbar_title = tk.Label(left, text="Patient Dashboard",
                                     bg=CLR_WHITE, fg=CLR_TEXT, font=FONT_H1)
        self.topbar_title.pack(anchor=tk.W)
        today = datetime.datetime.now().strftime("%A, %d %B %Y")
        tk.Label(left, text=today,
        ...
        tk.Button(right, text="＋  Book Appointment",
        ...
        tk.Label(right, text="🔔", bg=CLR_WHITE,
                 font=("Segoe UI Emoji", 14), cursor="hand2").pack(side=tk.LEFT, padx=4)
    ...
    def _show_page(self, name):
        ...
        if name not in self.pages:
            ...
            if name == "Dashboard":
                ...
                tk.Label(frame, text=f"🚧  {name} — Coming Soon",
    ...
    def _welcome_banner(self, parent):
        ...
        tk.Label(left, text=f"{greeting}, {self.patient['name']} 👋",
                 bg=CLR_PRIMARY, fg=CLR_WHITE,
                 font=("Segoe UI", 15, "bold")).pack(anchor=tk.W)
        tk.Label(left, text=f"Patient ID: {self.patient['id']}  •  Welcome to your HOPIFY portal.",
                 bg=CLR_PRIMARY, fg="#B8D4FF", font=FONT_BODY).pack(anchor=tk.W, pady=(4, 10))
        tk.Button(left, text="Book Appointment →",
                  bg=CLR_ACCENT, fg=CLR_WHITE,
                  font=("Segoe UI", 10, "bold"),
                  relief=tk.FLAT, cursor="hand2",
                  padx=12, pady=5,
                  activebackground="#00a88e",
                  activeforeground=CLR_WHITE,
                  command=self._book_appointment_dialog
                  ).pack(anchor=tk.W)
    ...
    def _quick_stats(self, parent):
        tk.Label(parent, text="Quick Overview",
                 bg=CLR_BG, fg=CLR_MUTED, font=("Segoe UI", 10, "bold")
                 ).pack(anchor=tk.W, padx=28, pady=(22, 8))
    ...
    def _stat_card(self, parent, icon, title, value, sub, color, col):
        card = tk.Frame(parent, bg=CLR_WHITE, padx=18, pady=16,
        ...
        tk.Label(icon_bg, text=icon, bg=color,
                 font=("Segoe UI Emoji", 16)).place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        tk.Label(top, text=title, bg=CLR_WHITE,
                 fg=CLR_MUTED, font=FONT_SMALL,
                 wraplength=110, justify=tk.LEFT).pack(side=tk.LEFT, padx=10, anchor=tk.W)
    ...
    def _upcoming_appointments_card(self, parent):
        ...
        tk.Button(empty, text="＋  Book Appointment",
                  bg=CLR_PRIMARY, fg=CLR_WHITE,
                  font=("Segoe UI", 10, "bold"),
                  relief=tk.FLAT, cursor="hand2",
                  padx=14, pady=7,
    ...
    def _build_profile_page(self, parent):
        ...
        for lbl_text, val in [("Blood Type", self.patient["blood"]), ("Age", str(self.patient["age"]) + " years"), ("Gender", self.patient["gender"])]:
        ...
        tk.Button(form, text="💾  Save Changes",
                  ...
                  activebackground="#1558b0",
                  activeforeground=CLR_WHITE,
                  command=lambda: messagebox.showinfo("HOPIFY", "✅ Profile updated successfully!")
    ...
    def _tab_bar(self, parent, tabs, on_select):
        ...
        for i, tab in enumerate(tabs):
            is_first = (i == 0)
            f = tk.Frame(parent, bg=CLR_PRIMARY if is_first else CLR_WHITE, highlightbackground=CLR_PRIMARY if is_first else CLR_BORDER, highlightthickness=1, cursor="hand2", padx=14, pady=6)
            f.pack(side=tk.LEFT, padx=(0, 6))
            lbl = tk.Label(f, text=tab, bg=CLR_PRIMARY if is_first else CLR_WHITE, fg=CLR_WHITE if is_first else CLR_MUTED, font=("Segoe UI", 9, "bold") if is_first else ("Segoe UI", 9))
            lbl.pack()
    ...
    def _book_appointment_dialog(self):
        ...
        tk.Button(btn_frame, text="Cancel", bg=CLR_BG, fg=CLR_MUTED, font=FONT_BODY, relief=tk.FLAT, padx=14, pady=8, cursor="hand2", command=dialog.destroy).pack(side=tk.LEFT)
        tk.Button(btn_frame, text="✔  Submit Request",
                  bg=CLR_PRIMARY, fg=CLR_WHITE,
                  font=("Segoe UI", 10, "bold"),
...
if __name__ == "__main__":
    if not USE_BOOTSTRAP:
        print("ℹ️  ttkbootstrap not found — running with plain tkinter.")
        print("   To install: pip install ttkbootstrap")
